function PathToSupport() {
  return (
    <div className="bg-gray-50 py-[130px] px-[110px] space-y-[60px]">
...
function YourPathCard({ img, imgClass="", title, classes, points }) {
  return (
    <div
      className={
        "flex flex-col border border-black rounded-[30px] p-[14px] space-y-[13px] " + classes
      }
    >
      <img src={img} alt={title} className={"rounded-[30px] w-full " + imgClass} />

      <div className="grow flex flex-col space-y-[13px] p-1">
        <h2 className="text-[30px] font-miniature">{title}</h2>

        <ul className="grow list-disc text-[18px] leading-[22px] pl-[23px] space-y-[22px]">
          {points?.map((point, id) => (
            <li
              key={id}
